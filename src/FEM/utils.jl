...
using u_Nodes::Array{Float64, 1} = getSolAtElement(u, element, problemDim) and the gradient of the shape function at the point using 
...
"""
 function get_∂u_∂x!(∂u_∂x::Array{Float64, 2}, u_Nodes::Array{Float64, 1}, ∂ϕ_∂x::AbstractArray{Float64,2}, problemDim::Int64)
    fill!(∂u_∂x, 0.0)
    for a ∈ 1:size(∂ϕ_∂x, 1)
        for J ∈ 1:size(∂ϕ_∂x, 2)
            for i ∈ 1:problemDim
...
function get_∂u_∂x(u_Nodes::Array{Float64, 1}, ∂ϕ_∂x::Array{Float64,2}, problemDim::Int64)
...
initialize_∂u_∂x(∂ϕ_∂x::Array{Float64,2}, problemDim::Int64) = zeros(problemDim, size(∂ϕ_∂x, 2))
...
using u_Nodes::Array{Float64, 1} = getSolAtElement(u, element, problemDim) and the shape function at the point using 
ϕ::Array{Float64,1} = RapidFEM.get_ϕ(shapeFunction, ipNo)

u is of size problemDim

get_u!(u::Array{Float64, 1}, u_Nodes::Array{Float64, 1}, ϕ::Array{Float64,1}, problemDim::Int64)"""
...
u  = get_u(u_Nodes::Array{Float64, 1}, ϕ::Array{Float64,1}, problemDim::Int64)"""

function get_u(u_Nodes::Array{Float64, 1}, ϕ::Array{Float64,1}, problemDim::Int64)
    u = zeros(problemDim)
...
using u_Nodes::Array{Float64, 1} = getSolAtElement(u, element, problemDim) and the coord matrix of the element from 
...
"""
function getCurrentCoordArray!(currentCoordArray::Array{Float64}, coordArray::Array{Float64}, u_Nodes::Array{Float64, 1})
...
"""
function getCurrentCoordArray(coordArray::Array{Float64}, u_Nodes::Array{Float64, 1})
